...
@section('content')
    <div class="mb-sm-4 d-flex flex-wrap align-items-center text-head">
        <h2 class="mb-3 me-auto">Menu List</h2>
        <div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        ...
        <div class="d-flex align-items-center flex-wrap">
            <button class="btn btn-primary btn-rounded me-3 mb-2" id="new-food-menu" value="{{ route('addmenu') }}"><i
                    class="fas fa-user-friends me-2"></i>+ Add Menu</button>
            <a href="{{ route('menu') }}" class="btn btn-secondary btn-rounded mb-2"><i class="fas fa-sync"></i></a>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-12">
                ...
                <table class="table display mb-4 dataTablesCard order-table shadow-hover  card-table text-black"
                    ...
                    <tbody>
                        @foreach ($menu as $list)
                            <tr class="table-tr" href="{{ route('editmenu', ['id' => $list->id]) }}">
                                <td class="tbl-bx">
                                    <div class="form-check ms-2">
                                ...
                                <td><a href="{{ route('editmenu', ['id' => $list->id]) }}"><?= $list['id'] ?></a></td>
                                <td><?= $list['name'] ?></td>
                                <td><?= $list['description'] ?></td>
                                <td class="text-center"><img class="menu-img"
                                        src="{{ asset('/public/uploads/menu/' . $list->id . '/' . $list->cover_image) }}">
                                </td>
                                <td><?= '$' . $list['price'] ?></td>
                                <td><?= empty($list->menu->name) ? '-' : $list->menu->name ?></td>
                                <?php
                                if ($list->status == 1) {
                                    ...
                                    <div class="dropdown ml-auto">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item text-black"
                                                href="{{ route('editmenu', ['id' => $list->id]) }}"><i
                                                    class="far fa-check-circle me-1 text-success"></i>Edit</a>
                                            <a class="dropdown-item text-black"
                                                href="{{ route('delete-menu', ['id' => $list->id]) }}"><i
                                                    class="far fa-times-circle me-1 text-danger"></i>Delete</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
<div class="modal fade" id="modal-new-menu" style="display:none">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header bg-light-blue">
                <h4 class="modal-title">New Menu</h4>
            </div>
            <div class="modal-body modal-md" role="document" id="new-menu-details">
            </div>
        </div>
    </div>
</div>
...
<script>
    $(function() {
        $('#new-food-menu').click(function() {
            $('#modal-new-menu').modal('show')
                .find('#new-menu-details')
